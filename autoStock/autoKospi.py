...
def kospi_trade():  # 대신증권 크레온으로 주식거래
    ...
    for idx, data in enumerate(res):  # 데이터 복수 일 수 있음
        ...
        if now_people > last_people:
            # 기관, 외국인 감소
            if now_for < last_for and now_ins < last_ins:
                add_sell(2)  # 매도포지션 증가
...
def haveStock():
    # 둘중 하나를 보유중이면 True
    if now_leverage is True or now_inverse is True:
        return True
...
def isReal():
    ...
    # 주식 미보유 중이면 추세 판단하지 않고 무조건 진짜로 받아들임
    # res = haveStock()
    # if res is False:
    #     return True
...
def chkRightTrade():
    if now_leverage is True:
        chkLeverage()

    if now_inverse is True:
        chkInverse()

def chkLeverage():
    before_p = 1.0

    for code in etfLeverage:
        # 레버리지 현재가
        now_p = daishinTrade.getNowPrice(code)
        time.sleep(1)
        ...
        #보정값 생성
        # alpha = math.floor(before_p * 0.0125)
        #alpha = 5

        #소수점 버림
        # before_p = math.floor(before_p)

        # 시장가로 매수하므로 한단계 5를 보정해줌
        writeLog(now_p, " <-L-> ", before_p)
        if (before_p / now_p) > SELL_MINUS:
            sell_leverage_only('MINUS')
        elif (now_p / before_p) > SELL_PLUS:
            sell_leverage_only('PLUS')

def chkInverse():
    before_p = 1.0

    for code in etfInverse:
        # 레버리지 현재가
        now_p = daishinTrade.getNowPrice(code)
        time.sleep(1)
        ...
        # 보정값 생성
        # alpha = math.floor(before_p * 0.0025)
        # alpha = 5
        ...
        # 시장가로 매수하므로 한단계 5를 보정해줌
        writeLog(now_p, " <-I-> ", before_p)
        if (before_p / now_p) > SELL_MINUS:
            sell_inverse_only('MINUS')
        elif (now_p / before_p) > SELL_PLUS:
            sell_inverse_only('PLUS')


def inverseOn():
    global now_inverse
    # global inverse_kospi

    if now_inverse is False:
        now_inverse = True
        # inverse_kospi = now_kospi
...
def leverageOn():
    global now_leverage
    # global leverage_kospi

    if now_leverage is False:
        now_leverage = True
        # leverage_kospi = now_kospi
...
def add_buy(cnt):
    ...
    res = isReal()
    if res is False:
        writeLog(dt, '[PASS] - [매수]레버리지 매수 & 인버스매도 ', dev_gap_peo_list[-1], ",", dev_avg)
        return
    else:
        buy_cnt = buy_cnt + cnt
        sell_cnt = 0
        ...
        # 테스트인 경우
        if isTest is False:
            writeLog(dt,'매수시점 증가 ', buy_cnt, ', ', sell_cnt, ', ', just_cnt)
...
def add_sell(cnt):
    ...
    res = isReal()
    if res is False:
        writeLog(dt, '[PASS] - [매도]인버스매수 & 레버리지매도 ', dev_gap_peo_list[-1], ",", dev_avg)
        return
    else:
        buy_cnt = 0
        ...
        just_cnt = 0
        if isTest is False:
            writeLog(dt,'매도시점 증가 ', buy_cnt, ', ', sell_cnt, ', ', just_cnt)
...
def add_just(cnt):
    ...
    if isTest is False:
        writeLog(dt,'관망시점 증가 ', buy_cnt, ', ', sell_cnt, ', ', just_cnt)
...
def sell_inverse_only(status):
    inverseOff()
    writeLog(dt, '[매도]인버스 Only매도 : ',status)
    if isTest:
        return
    ...
    for etf in etfInverse:  # 인버스 대상매도
        if etf in stock_code:
            ...
            writeLog(" - 인버스Only(", etf, ") 매도 : ", amount, '주, 매입가격 : ', buyPrice)
            ...
            writeLog(" - 인버스Only 매도(", etf, ")는 잔고에 존재하지 않습니다.")


def sell_leverage_only(status):
    leverageOff()
    writeLog(dt, '[매도]레버리지 Only매도 : ',status)
    # 테스트인 경우
    if isTest:
        return
    ...
    for etf in etfLeverage:  # 레버리지
        if etf in stock_code:
            ...
            writeLog(" - 레버리지Only(", etf, ") 매도 : ", amount, '주, 매입가격 : ', buyPrice)
            ...
            writeLog(" - 레버리지Only(", etf, ")는 잔고에 존재하지않습니다.")
